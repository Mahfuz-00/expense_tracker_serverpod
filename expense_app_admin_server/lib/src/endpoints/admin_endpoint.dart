...
import '../features/notification/business/push_service.dart';
...
import 'package:serverpod/serverpod.dart';
import '../features/expenses/business/expense_manager.dart';
...
class AdminEndpoint extends Endpoint {
  ...
  Future<List<Expense>> getExpenses(Session session) async {
    ...
    return await expenseManager.getAllExpenses();
  }

  @override
  Future<void> handleStreamMessage(StreamingSession session, SerializableModel message) async {
    if (message is UserIdentity) {
      session.messages.addListener('user_channel_${message.uuid}', (msg) {
        sendStreamMessage(session, msg);
      });
    }
  }
  ...
  Future<void> sendBudgetAlert(Session session, String userId, String type) async {
    // Just a "pass-through" to the Business logic
    PushService.sendBudgetAlert(session, userId, type);
  }
}