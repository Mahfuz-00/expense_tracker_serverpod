...
class SupabaseService {
  ...
  static SupabaseClient get client {
    if (_client == null) {
      ...
      try {
        // 1. Try to get it from the standard Serverpod way first
        key = Serverpod.instance!.getPassword('supabaseServiceKey');

        // 2. If null, manually read your specific fixed path
        if (key == null) {
          final file = File('lib/src/config/passwords.yaml');
          if (file.existsSync()) {
            final yamlString = file.readAsStringSync();
            final doc = loadYaml(yamlString);
            key = doc['supabaseServiceKey'];
          }
        }
      } catch (e) {
      ...
      if (key == null) {
        throw Exception("Could not find 'supabaseServiceKey' in lib/src/config/passwords.yaml");
      }
